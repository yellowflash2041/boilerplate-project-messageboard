    ...
    test("Viewing the 10 most recent threads with 3 replies each: GET request to '/api/threads/{board}'", () => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          assert.exists(res.body[0].delete_password);
        });
    }),
        ...
        .end((err, res) => {
          chai.request(server)
            ...
            .end((error, response) => {
              ...
              assert.equal(response.body.replies.length, 1);
              assert.equal(response.body.replies[0].text, 'This is the new reply to the new thread: POST request');
              assert.equal(response.body.replies[0].delete_password, 'thereply');
